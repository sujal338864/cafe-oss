...
export const createCategory = async (req: AuthRequest, res: Response) => {
  ...
  const existing = await categoryService.getCategoryByName(req.user!.shopId, name);
  if (existing) {
...
export const updateCategory = async (req: AuthRequest, res: Response) => {
  const category = await categoryService.getCategoryById(req.params.id, req.user!.shopId);
  if (!category) {
